...
async function syncNow(){
  ...
  try{
    const[co,fa,sc,ca,dj,mg]=await Promise.all([
...
function updateClock(){document.getElementById('clk').textContent=new Date().toLocaleDateString('it-IT',{weekday:'short',day:'2-digit',month:'short'});}
...
function _updateMargine(){
  ...
  const ore=(cd.ore||[]).reduce((s,o)=>s+(parseFloat(o.ore||0)*parseFloat(o.tariffa||0)),0);
  const costiTot=costiDist+ore;
  const margine=importoOff>0?((importoOff-costiTot)/importoOff*100):null;
  const col=margine===null?'var(--text3)':margine>=30?'var(--green)':margine>=15?'var(--amber)':'var(--red)';
  el.innerHTML=`<div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">Margine</div>
    <div style="font-size:22px;font-weight:800;color:${col}">${margine!==null?margine.toFixed(0)+'%':'—'}</div>
    <div style="font-size:11px;color:var(--text3);margin-top:2px">Costi: €${fmt2(costiTot)} · Offerta: €${fmt2(importoOff)}</div>
    ${importoOff>0?`<div class="margine-bar" style="margin-top:6px"><div class="margine-fill ${margine<15?'low':margine<30?'mid':''}" style="width:${Math.min(100,Math.max(0,margine||0))}%"></div></div>`:''}`;
}
...
// Auto-init quando caricato come modulo Hugo
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function() {
    if (typeof initApp === 'function') initApp();
    else if (typeof navTo === 'function') navTo('home');
  });
}
